...
public:
    ...
    ~StackWalkHelper();
    bool stackWalkOtherThread(HANDLE hThread, unsigned int entryPointTime1, volatile int & entryPointTime2, callstack::StackWalkResult* output);
...
private:
    ...
    HANDLE _wakeupEvent;
    volatile int _mainThreadSuspend;
    volatile int _mainThreadSuspendTimePoint;
    volatile int _stackWalkTimeout;
    volatile int _mainThread;
};
